...
- [x] 1.1 在仓库根目录新增 `packages/food_app_common/` Flutter/Dart package（仅 Dart 也可），并确认 `apps/food_app` 以 path 依赖方式引用
- [x] 1.2 在 `packages/food_app_common/pubspec.yaml` 声明公共模块所需依赖（dio、connectivity_plus、webview_flutter、package_info_plus、device_info_plus、logger、appsflyer_sdk、adjust_sdk、app_tracking_transparency 等）
- [x] 1.3 在 `packages/food_app_common/lib/` 创建目录骨架：`boot/ remote_config/ analytics/ web_shells/ att/ routing/`
- [x] 1.4 定义公共包对外 export（例如 `food_app_common.dart`）与最小可编译占位实现

## 2. 抽离 Remote Config（endpoint 注入）

- [x] 2.1 将 `apps/food_app/lib/models/remote_config.dart` 迁移到 `packages/food_app_common/lib/remote_config/remote_config.dart`
...
- [x] 3.1 将 `apps/food_app/lib/boot/boot_decision.dart` 迁移到 `packages/food_app_common/lib/boot/boot_decision.dart`
- [x] 3.2 将 `apps/food_app/lib/boot/boot_coordinator.dart` 迁移到 `packages/food_app_common/lib/boot/boot_coordinator.dart`
...
- [x] 4.1 将 `apps/food_app/lib/analytics/analytics_bridge.dart` 迁移到 `packages/food_app_common/lib/analytics/analytics_bridge.dart`
...
- [x] 5.1 将 `apps/food_app/lib/shells/js_message.dart`、`js_payload.dart` 迁移到 `packages/food_app_common/lib/web_shells/`
- [x] 5.2 将 `apps/food_app/lib/shells/web_shell_one_page.dart` 迁移到 `packages/food_app_common/lib/web_shells/web_shell_one_page.dart`
- [x] 5.3 将 `apps/food_app/lib/shells/web_shell_two_page.dart` 迁移到 `packages/food_app_common/lib/web_shells/web_shell_two_page.dart`
...
- [x] 6.1 将 `apps/food_app/lib/services/att_service.dart` 迁移到 `packages/food_app_common/lib/att/att_service.dart`
...
- [x] 7.1 在 `apps/food_app/pubspec.yaml` 增加对 `packages/food_app_common` 的 path 依赖，并确保依赖解析通过
- [x] 7.2 更新 `apps/food_app/lib/*` 的 imports：统一从 `package:food_app_common/...` 引用抽离后的能力
