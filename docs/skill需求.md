...
2. 流程：要求输入马甲包类型（工具包、游戏包），如果没有则随机定一个包类型 -> 在apps目录中生成应用，需要集成 food_app_common 相关逻辑 -> 检查 Info.plist 相关权限是否定义（要求一定有ATT）

3. Cursor Skills（项目级）：

- `.cursor/skills/jacket-flutter-app-create`：创建 Flutter iOS 马甲包骨架 + 集成 `food_app_common` 的启动链路
