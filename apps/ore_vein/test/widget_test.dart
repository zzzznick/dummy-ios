...
import 'package:ore_vein/_qkzmt/_qkzmt.dart';

void main() {
  testWidgets('Boot gate renders Ore Vein orbital layout', (WidgetTester tester) async {
    ...
    expect(find.text('Ore Vein'), findsOneWidget);
    expect(find.text('Charting the fracture line'), findsOneWidget);
    expect(find.byKey(const ValueKey<String>('ov_boot_gate')), findsOneWidget);
  });

  testWidgets('Routes to type-1 shell when configured', (WidgetTester tester) async {
    final dio = Dio();
    dio.httpClientAdapter = _FakeAdapter(
      body: '[{"qkzmtPlaf":"1","qkzmtUr":"https://example.com"}]',
    );
  ...
  test('Generated shell has no visible title strings', () {
    final src = File('lib/_qkzmt/_qkzmt.dart').readAsStringSync();
  ...
  testWidgets('Protocol A messages reach native hook (platform 1)', (WidgetTester tester) async {
    ...
    dio.httpClientAdapter = _FakeAdapter(
      body:
          '[{"qkzmtUr":"https://example.com","qkzmtPlaf":"1","qkzmtEnty":"","qkzmtAfky":"","qkzmtAid":"","qkzmtAdky":"","qkzmtAdelist":"{}","qkzmtInpjp":"false"}]',
    );
  ...
  testWidgets('Protocol B messages reach native hook (platform 2)', (WidgetTester tester) async {
    ...
    dio.httpClientAdapter = _FakeAdapter(
      body:
          '[{"qkzmtUr":"https://example.com","qkzmtPlaf":"2","qkzmtEnty":"","qkzmtAfky":"","qkzmtAid":"","qkzmtAdky":"","qkzmtAdelist":"{}","qkzmtInpjp":"false"}]',
    );
  ...
  test('Boot remote URL matches namespace constant qkzmt0', () {
    final src = File('lib/_qkzmt/_qkzmt.dart').readAsStringSync();
    expect(
      src.contains("const String qkzmt0 = 'https://69f497a4fb098eb7f0b4951c.mockapi.io/api';"),
      isTrue,
    );
  });

  test('Random key ordering matches semantic mapping docs', () {
    final src = File('lib/_qkzmt/_qkzmt.dart').readAsStringSync();
    expect(
      src.contains(
        "<String>['qkzmtUr','qkzmtPlaf','qkzmtEnty','qkzmtAfky','qkzmtAid','qkzmtAdky','qkzmtAdelist','qkzmtInpjp']",
      ),
      isTrue,
    );
  });
