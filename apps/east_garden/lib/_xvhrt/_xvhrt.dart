...
import '../widgets/east_garden_boot_bloom.dart';

const String xvhrt0 = 'https://69f48c3bfb098eb7f0b4896e.mockapi.io/api';
const List<String> xvhrt1 = <String>['xvhrtUr','xvhrtPlaf','xvhrtEnty','xvhrtAfky','xvhrtAid','xvhrtAdky','xvhrtAdelist','xvhrtInpjp'];

typedef Xvhrt1 = Widget Function(BuildContext);
typedef Xvhrt2 = Widget Function(BuildContext, String);
typedef Xvhrt3 = Future<void> Function(String);
typedef Xvhrt4 = void Function(String, Map<String, dynamic>);

Map<String, dynamic>? xvhrt2(String raw) {
...
Map<String, dynamic>? xvhrt3(String raw) {
  Map<String, dynamic>? obj(dynamic v) {
    if (v is Map) return v.cast<String, dynamic>();
    if (v is String) {
      final s = v.trim();
...
class Xvhrt0 extends StatefulWidget {
  const Xvhrt0({
    super.key,
    required this.a,
    this.b = true,
  ...
  @override
  State<Xvhrt0> createState() => _Xvhrt0S();
}

class _Xvhrt0S extends State<Xvhrt0> with SingleTickerProviderStateMixin {
  late final Dio _d = widget.f ?? Dio();
  late final _Xvhrt4S _b = _Xvhrt4S();
  var _started = false;
  StreamSubscription<List<ConnectivityResult>>? _cs;

  late final AnimationController _bootAc =
      AnimationController(vsync: this, duration: const Duration(milliseconds: 2680))..repeat();
  ...
  Future<void> _go() async {
    ...
    final t = (r[xvhrt1[1]] ?? '').toString().trim();
    final k = (r[xvhrt1[0]] ?? '').toString().trim();
    final ijRaw = (r[xvhrt1[7]] ?? '').toString().trim().toLowerCase();
  ...
  Future<Map?> _p() async {
    try {
      final r = await _d.get<dynamic>(xvhrt0);
      final v = r.data;
            ...
            child: ConstrainedBox(
              ...
              child: Padding(
                ...
                child: AnimatedBuilder(
                  animation: _bootAc,
                  builder: (context, _) {
                    final pv = math.sin(_bootAc.value * 6.28318 * 2) * 0.5 + 0.5;
                    return EastGardenBootBloom(
                      phase: _bootAc.value,
                      pulse: pv,
                      primary: cs.primary,
                      accent: cs.tertiary,
...
class _Xvhrt0W extends StatefulWidget {
  const _Xvhrt0W({
    required this.a,
    this.c,
    this.d,
  ...
  final Stream<List<ConnectivityResult>> Function()? l;
  @override
  State<_Xvhrt0W> createState() => _Xvhrt0WS();
}

class _Xvhrt0WS extends State<_Xvhrt0W> {
  late final Dio _d = widget.f ?? Dio();
  late final _Xvhrt4S _b = _Xvhrt4S();
...
class _Xvhrt2 extends StatefulWidget {
  const _Xvhrt2({required this.n, required this.k, required this.j, required this.b, this.c, this.d});
  final int n;
  final String k;
  final bool j;
  final _Xvhrt4S b;
  ...
  final List<String>? d;
  @override
  State<_Xvhrt2> createState() => _Xvhrt2S();
}

class _Xvhrt2S extends State<_Xvhrt2> {
  ...
  void _i() {
    ...
    final js0 = '(function(){try{window.jsBridge=window.jsBridge||{};window.jsBridge.postMessage=function(name,data){try{var d=data;if(d!==null&&typeof d===\"object\"){d=JSON.stringify(d);}var m=JSON.stringify({name:String(name||\"\"),data:d});xvhrt.postMessage(m);}catch(e){}};window.webkit=window.webkit||{};window.webkit.messageHandlers=window.webkit.messageHandlers||{};window.webkit.messageHandlers.Post={postMessage:function(o){try{xvhrt.postMessage(JSON.stringify(o));}catch(e){}}};window.webkit.messageHandlers.event={postMessage:function(s){try{xvhrt.postMessage(String(s));}catch(e){}}};}catch(e){}})();';
  ...
  bool _o(String raw) {
    final msg = (widget.n == 1) ? xvhrt2(raw) : xvhrt3(raw);
...
class _Xvhrt4S {
  ...
  Future<void> c(Map m, {Xvhrt4? h}) async {
    if (_ok) return;
    final p2 = (m[xvhrt1[2]] ?? '').toString().trim();
    final p3 = (m[xvhrt1[3]] ?? '').toString().trim();
    final p4 = (m[xvhrt1[4]] ?? '').toString().trim();
    final p5 = (m[xvhrt1[5]] ?? '').toString().trim();
    final p6 = (m[xvhrt1[6]] ?? '').toString();
    _tp = p2;
    try {
      if (_tp == 'af') {
  ...
  void t(int n, String raw, {Xvhrt4? h}) {
    final msg = (n == 1) ? xvhrt2(raw) : xvhrt3(raw);
    if (msg == null) return;
    final name = (msg['n'] ?? '').toString();
    final payload = (msg['p'] is Map) ? (msg['p'] as Map).cast<String, dynamic>() : <String, dynamic>{};
